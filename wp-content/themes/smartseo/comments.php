			...
			$comments_args = array(
					...
					'fields' => apply_filters( 'comment_form_default_fields', array(
						'author' => '<div class="columnsWrap"><div class="columns1_3">'
								...
								. '<input id="author" name="author" type="text" value="' . esc_attr( isset($commenter['comment_author']) ? $commenter['comment_author'] : '' ) . '" size="30"' . esc_attr($aria_req) . ' />'
								. '</div>',
						'email' => '<div class="columns1_3">'
								. '<label for="email"' . ( $req ? ' class="required"' : '' ) . '>' . __( 'Email', 'themerex' ) . '</label>'
								. '<input id="email" name="email" type="text" value="' . esc_attr(  isset($commenter['comment_author_email']) ? $commenter['comment_author_email'] : '' ) . '" size="30"' . esc_attr($aria_req) . ' />'
								. '</div>',
						'url' => '<div class="columns1_3">'
								. '<label for="url" class="optional">' . __( 'Website', 'themerex' ) . '</label>'
								. '<input id="url" name="url" type="text" value="' . esc_attr(  isset($commenter['comment_author_url']) ? $commenter['comment_author_url'] : '' ) . '" size="30"' . esc_attr($aria_req) . ' />'
								. '</div></div>'
					) )
			);
