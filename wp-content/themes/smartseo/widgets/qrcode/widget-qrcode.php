	...
	function widget($args, $instance) {
		...
		if ($show_personal) 
			$output .= '
					...
					' . ($show_what==1 
						? '<p class="user_name odd first"><span class="theme_text">' . __('Name:', 'themerex') . '</span> <span class="theme_info">' . esc_html($ufname) . ' ' . esc_html($ulname) . '<span></p>'
							. ($ucompany ? '<p class="user_company even"><span class="theme_text">' . __('Company:', 'themerex') . '</span> <span class="theme_info">' . esc_html($ucompany) . '<span></p>' : '')
							. ($uphone ? '<p class="user_phone odd"><span class="theme_text">' . __('Phone:', 'themerex') . '</span> <span class="theme_info">' . esc_html($uphone) . '<span></p>' : '')
							. ($uemail ? '<p class="user_email even"><span class="theme_text">' . __('E-mail:', 'themerex') . '</span> <a href="mailto:' . esc_url($uemail) . '">' . esc_html($uemail) . '</a></p>' : '')
							. ($usite ? '<p class="user_site odd"><span class="theme_text">' . __('Site:', 'themerex') . '</span> <a href="' . esc_url($usite) . '" target="_blank">' . esc_html($usite) . '</a></p>' : '')
						: $text)
						. '
					</div>
					';
				...
				<?php if ($instance['show_what']==1) { ?>
                    ...
                    <?php 
						echo  ($instance['ucompany'] ? '<li class="user_company even">' . __('Company:', 'themerex') . ' ' . ($instance['ucompany']) . '</li>' : '')
							...
							. ($instance['usite'] ? '<li class="user_site odd">' . __('Site:', 'themerex') . ' ' . '<a href="' . esc_url($instance['usite']) . '" target="_blank">' . esc_html($instance['usite']) . '</a></li>' : '');
					?>
				<?php } else { ?>
