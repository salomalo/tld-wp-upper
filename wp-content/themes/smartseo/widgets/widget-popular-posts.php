	...
	function widget($args, $instance) {
		...
		for ($i=0; $i<3; $i++) {
			...
			if (have_posts()) {
				if ($THEMEREX_CURRENT_SIDEBAR == 'top') {
					$output .= '
						<div class="tab_content columns1_3" id="widget_popular_' . esc_attr($i) . '">
							<h2 class="widgetSubtitle">' . esc_html($title_tabs[$i]) . '</h2>
					';
				} else {
					$tabs .= '<li class="squareButtonlite"><a href="#widget_popular_' . esc_attr($rnd) . '_' . esc_attr($i) . '">' . esc_html($title_tabs[$i]) . '</a></li>';
					$output .= '
						<div class="tab_content" id="widget_popular_' . esc_attr($rnd) . '_' . esc_attr($i) . '"'.($i==1 && !$output ? ' style="display: block;"' : '').'>
					';
				}
				$post_number = 0;
