...
function themerex_options_show_field($field, $value=null) {
	...
	if (themerex_options_is_group($field['type'])) {					// Close nested containers
		...
		if ($field['type'] != 'hidden') {
			echo '<div class="themerex_options_field'
				...
				. esc_attr($divider)
				. '">'."\n";
			echo '<label class="themerex_options_field_label'.(!empty($THEMEREX_flags['add_inherit']) && isset($field['std']) ? ' themerex_options_field_label_inherit' : '').'" for="' . esc_attr($field['id']) . '">' . esc_attr($field['title'])
	...
	foreach ($field['val'] as $clone_num => $clone_val) {
		...
		case 'tab':
			...
			if (!empty($field['start']))
				echo '<div id="' . esc_attr($field['start']) . '" class="themerex_options_container themerex_options_tab'.esc_attr($divider).'">'
					. '<ul>' . balanceTags(themerex_options_collect_tabs($field['type'], $field['start'])) . '</ul>';
		...
		case 'date':
			if (isset($field['style']) && $field['style']=='inline') {
				echo '<div class="themerex_options_input_date" id="'. esc_attr($field['id']) .'_calendar"'
					...
					. ' data-linked-field="' . (!empty($data['linked_field']) ? $data['linked_field'] : esc_attr($field['id'])) . '"'
					. '></div>'
		...
		case 'text':
			echo '<input class="themerex_options_input themerex_options_input_text' . (!empty($field['mask']) ? ' themerex_options_input_masked' : '') . '"'
				...
				. ' id="'. esc_attr($field['id']) .'"'
				. ' type="text"'
				. ' value="'. esc_attr(htmlspecialchars(is_inherit_option($clone_val) ? '' : $clone_val)) . '"'
			...
			if (!is_inherit_option($clone_val)) {
				...
				if (count($tags) > 0) {
					foreach($tags as $tag) {
						...
						echo '<span class="themerex_options_tag iconadmin-cancel">'.esc_html($tag).'</span>';
					}
				}
			}
		...
		case 'socials':
			...
			echo balanceTags($list);
			echo '<input name="'. esc_attr($field['id']) . '_icon' . ($cloneable ? '[]' : '') .'" type="hidden" value="'. htmlspecialchars(is_inherit_option($clone_val['icon']) ? '' : $clone_val['icon']) . '" />';
		break;
	...
	if (!themerex_options_is_group($field['type']) && $field['type'] != 'hidden') {
		...
		echo '</div>';
		if (!empty($field['desc']))
			echo '<div class="themerex_options_desc">' . esc_html($field['desc']) .'</div>' . "\n";
		echo '</div>' . "\n";
	}
}
