	...
	if (isset($form_fields['media_folder'])) {
		...
		if (count($terms) > 0) {
			foreach ($terms as $slug=>$name) {
				...
				$html .= ($html ? '<br />' : '') . '<input type="checkbox" class="text" id="media_folder_'.esc_attr($id).'" name="media_folder_' . esc_attr($slug) . '" value="' . esc_attr( $slug ) . '"' . (in_array($slug, $values) ? ' checked="checked"' : '' ) . ' ' . esc_attr($readonly) . ' ' . esc_attr($aria_required) . ' /><label for="media_folder_'.esc_attr($id).'"> ' . esc_html($name) . '</label>';
			}
		}
		$form_fields['media_folder']['input'] = 'media_folder_input';
		$form_fields['media_folder']['media_folder_input'] = '<div class="media_folder_selector">' . balanceTags($html) . '</div>';
	}
	return $form_fields;
}
