		...
		$output .= '
            <div class="infoTotal">
                ...
                <div class="totalWrap">
                    ...
                    <div class="totalRating" data-mark="'.esc_attr($avg).'">'.esc_html($avg).(themerex_strlen($avg)==1 ? '.0' : '').'</div>
                </div>
            </div>
		';
		return $output;
...
if ( !function_exists( 'getReviewsSummaryStars' ) ) {
	function getReviewsSummaryStars($avg, $editable=false, $snippets=false) {
		...
		$output = '<div class="ratingItem' . ($editable ? ' ratingEdit' : '') . '"'
			...
			. '>'
			. ($snippets ? '<meta itemprop="worstRating" content="0"><meta itemprop="bestRating" content="'.esc_attr($maxLevel).'"><meta itemprop="ratingValue" content="'.esc_attr($avg).'">' : '');
		if (!$editable && get_theme_option('reviews_style')=='text') {
			...
			$output .= ($editable ? '<span class="ratBubble">' . esc_html($avg) . '</span>' : '')
				. '<div class="starDefault">' . balanceTags($stars) . '</div>'
				. '<div class="starHover" style="width:'.($avg/$maxLevel*100).'%">' . balanceTags($stars) . '</div>';
		}
		if ($editable)	$output .= '<input type="hidden" name="reviews_marks[]" value="'. esc_attr($avg) . '" />';
		$output .= '</div>';
		return $output;
	}
}
