	...
	function start_el(&$output, $item, $depth=0, $args=array(), $current_object_id=0) {
		...
		if ($top_level_type == 'auto') { // If auto method set
			...
			if ($depth == 0 && $item->top_menu_view != 'default') {
				$class_names .= ' ' . esc_attr($item->top_menu_view) . ' custom_view_item';
			}
			$class_names = ' class="menu-item-has-children '. esc_attr( $class_names ) . '"';
			$item_output .= '<li'.esc_attr($class_names).'>';
			...
			$item_output .= '<a ' . esc_attr($attributes) . '>'
				. (!empty($item->item_icon_class) ? '<span class="menu_icon '.esc_attr($item->item_icon_class).'"></span>' : '')
			...
			if (!empty($inner_output)) {
				...
				if($view_type == 'columns') {
					...
					foreach ($inner_output as $cat => $cat_val) {
						...
						$list_num++;
						$item_output .= '<a href="'.esc_url($cat_val['term_link']).'">'.esc_html($cat_val['term_name']).'</a><ul>';
						foreach ($cat_val as $item_id => $item_data) {
							if($item_id == 'term_link' || $item_id == 'term_name') continue;
							$item_output .= '<li><a href="'.esc_url($item_data['item_link']).'">'.esc_html($item_data['item_title']).'</a></li>';
						}
						$item_output .= '</ul></li>';
					}
					...
					foreach ($inner_output as $cat => $cat_val) {
						foreach ($cat_val as $item_id => $item_data) {
							...
							$item_output .= '<li><a href="'.esc_url($item_data['item_link']).'"
								data-author="'.esc_attr($item_author).'"
								data-pubdate="'.esc_attr($item_date).'"
								data-thumb="'.esc_attr($item_thumb_full).'"
								data-title="'.esc_attr($item_title).'"
								data-link="'.esc_attr($item_link).'"
					...
					if ($curr_item != '') {
						$item_output .= '
							<div class="thumb_wrap">'.balanceTags($placeholder_thumb).'</div>
							<h4 class="item_title"><a href="'.esc_url($placeholder_link).'">'.esc_html($placeholder_title).'</a></h4>
							<div class="item_info">
							<div class="item_pubdate"><span>'.__( 'Posted', 'themerex' ).'</span>&nbsp;<em>'.esc_html($placeholder_date).'</em></div>
							<div class="item_author"><span>'.__( 'by', 'themerex' ).'</span>&nbsp;<em>'.esc_html($placeholder_author).'</em></div>
							</div>
						';
					}
					...
					foreach ($inner_output as $cat => $cat_val) {
						$curr_item = '';
						$item_output .= '<li class="items_column"><a href="'.esc_url($cat_val['term_link']).'">'.esc_html($cat_val['term_name']).'</a><ul>';
						foreach ($cat_val as $item_id => $item_data) {
							if($item_id == 'term_link' || $item_id == 'term_name') continue;
							$item_date = get_the_time($df, $item_id);
							...
							$item_output .= '<li>'
								. '<a href="'.esc_url($item_data['item_link']).'"
									data-pubdate="'.esc_attr($item_date).'"
									data-comments="'.esc_attr($item_comments->approved).'"
									data-thumb="'.esc_attr($item_thumb).'"
									>'
									.esc_html($item_data['item_title'])
								.'</a>'
								.'</li>';
						}         
						$item_output .= '</ul>';
						...
						if ($curr_item != '') {
							$item_output .= '
								...
								<div class="item_info">
								<div class="item_pubdate"><span>'.__( 'Posted', 'themerex' ).'</span>&nbsp;<em>'.esc_html($placeholder_date).'</em></div>
								<div class="item_comments"><span class="icon icon-comment-1"></span>&nbsp;<em>'.esc_html($placeholder_comments).'</em></div>
								</div>';
						}
						$item_output .= '</div></li>';
					}
			...
			$output .= $indent .($this->top_menu_view == 'thumb' && $depth > 1 || $this->top_menu_view == 'thumb_title' && $depth > 2  ? '</li>' : ''). '<li id="menu-item-'. esc_attr($item->ID) . '"' . ($value) . ($class_names) .'>';
			...
			if ($this->top_menu_view == 'thumb' || $this->top_menu_view == 'thumb_title') {
				if ($depth > 0) {
					...
					$attributes .= !empty( $post_title ) ? ' data-title="'.esc_attr($post_title).'"' : $description_title;
					$attributes .= !empty( $post_author ) ? ' data-author="'.esc_attr($post_author).'"' : '';
					$attributes .= !empty( $placeholder_thumb ) ? ' data-thumb="'.esc_attr($placeholder_thumb).'"' : '';
					$attributes .= !empty( $post_pubdate ) ? ' data-pubdate="'.esc_attr($post_pubdate).'"' : '';
					$attributes .= !empty( $comment_count ) ? ' data-comments="'.esc_attr($comment_count).'"' : '';
			...
			//if ($this->top_menu_view == 'columns' && $depth > 1) {
				if (!empty($item->item_icon_class)) {
					$item_output .= '<span class="menu_icon '.esc_attr($item->item_icon_class).'"></span>';
				}
			//}
			
			if ($this->top_menu_view == 'thumb' && $depth > 0) {
				if ($item_thumb != '') {
					$item_output .= '<img src="'.esc_url($item_thumb).'" width="'.esc_attr($width).'" height="'.esc_attr($height).'" alt="">';
				}
			} else {
	...
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ($show_ul) {
			if ($depth == 0 && $menu_view != 'default') {
				$output .= '<ul class="menu-panel '.esc_attr($menu_view).'"><li>';
			}
			$output .= "\n$indent<ul class=\"".($depth == 0 && $menu_view != 'default' ? 'custom-menu-style '.esc_attr($menu_view). ' ' : '')."sub-menu\">\n";
		}
	}
			...
			if(!empty($this->level_holder)) {
				...
				if ($top_item->top_menu_view == 'thumb_title') {			
					$output .= '<div class="item_placeholder">
						<div class="thumb_wrap"><img src="'.esc_url($item_thumb).'" alt="" width="'.esc_attr($vis_width).'" height="'.esc_attr($vis_height).'" /></div>
						<div class="item_info">
						<div class="item_pubdate"><span>'.__( 'Posted', 'themerex' ).'</span>&nbsp;<em>'.esc_html($item_pubdate).'</em></div>
						<div class="item_comments"><span class="icon icon-comment-1"></span>&nbsp;<em>'.esc_html($comment_count).'</em></div>
						</div>
						</div>';
				} else if ($top_item->top_menu_view == 'thumb') {
					$this->thumb_placeholder = '<div class="item_placeholder">
						<div class="thumb_wrap"><img src="'.esc_url($item_thumb).'" alt="" width="'.esc_attr($vis_width).'" height="'.esc_attr($vis_height).'" /></div>
						<h4 class="item_title"><a href="'.esc_url($post_link).'">'.esc_html($post_title).'</a></h4>
						<div class="item_info">
						<div class="item_pubdate"><span>'.__( 'Posted', 'themerex' ).'</span>&nbsp;<em>'.esc_html($item_pubdate).'</em></div>
						' . (!empty($author_name) ? '<div class="item_author"><span>'.__( 'by', 'themerex' ).'</span>&nbsp;<em>'.esc_html($author_name).'</em></div>' : '') . '
						</div>
						</div>';
				}
			}
