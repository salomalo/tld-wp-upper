	...
	function start_el(&$output, $item, $depth=0, $args=array(), $current_object_id=0) {
	    ...
	    if (!$this->list_show++) {	    
			...
			foreach ($icons_array as $icon) { 
				echo '<li><span class="'.esc_attr($icon).'"></span></li>';
			}
			echo '</ul></div>';
		}
	            ...
	            <dt class="menu-item-handle">
	                ...
	                <span class="item-controls">
	                    ...
	                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item'); ?>" href="<?php
	                        echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . esc_attr($item_id) ) ) );
							...
							<p class="auto_options_panel">
								...
								<?php
									...
									foreach ($post_types as $type) {
										...
										id="'.esc_attr($type.$item_id).'" />
										
										<label for="'.esc_attr($type.$item_id).'">'.esc_html($type).'</label>';
									}
								?>
								</span>
	            ...
	            <?php } ?>
					...
					<div class="image_add_row">
						...
						<?php
							...
							data-linked-field="item_thumb_holder'.esc_attr($item_id).'"
						...
						<div class="item_img"><?php echo !empty($item->item_thumb) ? '<img src="'.esc_url($item->item_thumb).'" alt="">' : ''; ?></div>
					</div>                  
