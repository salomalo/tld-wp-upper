...
function sc_accordion($atts, $content=null){	
    ...
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '')
			...
			. ($s!='' ? ' style="'.esc_attr($s).'"' : '')
			. ' data-active="' . esc_attr($initial-1) . '"'
			. '>'
...
function sc_accordion_item($atts, $content=null) {
    ...
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '')
			...
			. ($THEMEREX_sc_accordion_show_counter ? '<span class="sc_items_counter">'.intval($THEMEREX_sc_accordion_counter).'</span>' : '')
			. esc_html($title)
...
function sc_audio($atts, $content = null) {
    ...
    $output = '<audio' . ($id ? ' id="' . esc_attr($id) . '"' : '') . (!empty($class) ? ' class="'.esc_attr($class).'"' : '')
 . ' src="' . esc_url($src) . '" class="sc_audio" ' . (sc_param_is_on($controls) ? ' controls="controls"' : '') . (sc_param_is_on($autoplay) && is_single() ? ' autoplay="autoplay"' : '') . ' width="' . esc_attr($width) . '" height="' . esc_attr($height) .'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'></audio>';
...
function sc_banner($atts, $content=null){	
    ...
    $output = empty($src) ? '' : ('<a href="' . (empty($link) ? '#' : $link) . '"' . ($id ? ' id="' . esc_attr($id) . '"' : '')
		. ' class="sc_banner' . (sc_param_is_on($popup) ? ' user-popup-link' : '')
		. (!empty($class) ? ' '.esc_attr($class) : '')
		. '"'
		. (!empty($target) ? ' target="' . esc_attr($target) . '"' : '')
		. (!empty($rel) ? ' rel="' . esc_attr($rel) . '"' : '')
		. ($align && $align!='none' ? ' sc_align' . esc_attr($align) : '')
		. ($s!='' ? ' style="'.esc_attr($s).'"' : '')
		. '>'
		. '<img src="' . esc_url($src) . '" class="sc_banner_image" alt="" />'
		. (trim($title) ? '<span class="sc_banner_title">' . esc_html($title) . '</span>' : '')
		. (trim($content) ? '<span class="sc_banner_content">' . balanceTags($content) . '</span>' : '')
		. '</a>');
...
function sc_br($atts, $content = null) {
    ...
    $output = '<br' . (in_array($clear, array('left', 'right', 'both')) ? ' style="clear:' . esc_attr($clear) . '"' : '') . ' />';
...
function sc_blogger($atts, $content=null){	
	...
	$output = ($style=='list' ? '<ul' : '<div')
			...
			. ' class="sc_blogger'
				...
				. ' style_' . (in_array($style, array('accordion_1', 'accordion_2')) ? 'accordion' : (themerex_strpos($style, 'image')!==false ? 'image style_' : '') . esc_attr($style))
			...
			if ($filters == 'categories') {			// Use categories as filter items
				...
				if (count($portfolio_list) > 0) {
					...
					foreach ($portfolio_list as $cat) {
						$filters_list .= '<li class="squareButton'.($cat->term_id==$cat_id ? ' active' : '').'"><a href="#" data-filter=".flt_'.esc_attr($cat->term_id).'">'.esc_html($cat->name).'</a></li>';
					}
				}
				...
				if (count($flt_ids) > 0) {
					...
					foreach ($flt_ids as $flt_id=>$flt_name) {
						$filters_list .= '<li class="squareButton"><a href="#" data-filter=".flt_'.esc_attr($flt_id).'">'.esc_html($flt_name).'</a></li>';
					}
				}
			}
			if ($filters_list) {
				$output .= '<script type="text/javascript">'
					. 'jQuery(document).ready(function () {'
						. 'jQuery("#'.esc_attr($id).' .isotopeFiltr").append("<ul>'.addslashes($filters_list).'</ul>");'
					. '});'
					. '</script>';
			}
		}
...
function sc_button($atts, $content=null){	
    ...
    $output = '<div class="sc_button sc_button_style_' . esc_attr($style) . ' sc_button_size_' . esc_attr($size) . ($align && $align!='none' ? ' align'.esc_attr($align) : '') . (!empty($class) ? ' '.esc_attr($class) : '')
 . ' ' . esc_attr($type) . 'Button' . (sc_param_is_on($fullsize) ? ' fullSize' : '') . ' ' . esc_attr($style) . ' ' . esc_attr($size) . ($icon!='' ? '  ico' : '') . '"'
		. ($s!='' ? ' style="'.esc_attr($s).'"' : '') . '>'
		. '<a href="' . (empty($link) ? '#' : $link) . '"' . (!empty($target) ? ' target="' . esc_attr($target) . '"' : '') . (!empty($rel) ? ' rel="' . esc_attr($rel) . '"' : '')
		. ($id ? ' id="' . esc_attr($id) . '"' : '')
		. ' class="' . ($icon ?  $icon : '') . (sc_param_is_on($popup) ? ' user-popup-link' : '') . '"'
		. ($color !== '' ? ' style="background-color:' . esc_attr($color) . '; border-color:'. esc_attr($color) .';"' : '')
...
function sc_chat($atts, $content=null){	
	...
	$content = do_shortcode($content);
	if (themerex_substr($content, 0, 2)!='<p') $content = '<p>' . esc_html($content) . '</p>';
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_chat' . (!empty($class) ? ' '.esc_attr($class) : '') . '"' . ($s ? ' style="'.esc_attr($s).'"' : '') . '>'
		. balanceTags($content)
		. ($title == '' ? '' : ('<p class="sc_quote_title">' . ($link!='' ? '<a href="'.esc_url($link).'">' : '') . esc_html($title) . ($link!='' ? '</a>' : '') . '</p>'))
		.'</div>';
...
function sc_columns($atts, $content=null){	
    ...
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="columnsWrap sc_columns sc_columns_count_' . esc_attr($count) . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>' . do_shortcode($content).'</div>';
...
function sc_column_item($atts, $content=null) {
	...
	$s = ($align !== '' ? 'text-align:' . esc_attr($align) . ';' : '')
		.($color !== '' ? 'color:' . esc_attr($color) . ';' : '')
		.($bg_color !== '' ? 'background-color:' . esc_attr($bg_color) . ';' : '')
		.($bg_image !== '' ? 'background-image:url(' . esc_attr($bg_image) . ');' : '')
		.($style);
	$span = max(1, min(4, (int) $span));
	$output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="columns'.($span > 1 ? $span : 1).'_'.esc_attr($THEMEREX_sc_columns_count).' sc_column_item sc_column_item_'.esc_attr($THEMEREX_sc_columns_counter)
					. (!empty($class) ? ' '.esc_attr($class) : '')
...
function sc_contact_form($atts, $content = null) {
	...
	$THEMEREX_sc_contact_form_counter = 0;
	$content = do_shortcode($content);
    $output =  '<div ' . ($id ? ' id="' . esc_attr($id) . '"' : '') . 'class="sc_contact_form sc_contact_form_'.($content ? 'custom' : 'contact').(!empty($class) ? ' '.esc_attr($class) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '') .'>'
		. ($title ? '<h1 class="title">' . esc_html($title) . '</h1>' : '')
		. ($description ? '<div class="description">' . esc_html($description) . '</div>' : '')
		. '<form' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' data-formtype="'.($content ? 'custom' : 'contact').'" method="post" action="' . ($action ? $action : $THEMEREX_ajax_url) . '">'
		. ($content != '' 
			? $content 
...
function sc_contact_form_item($atts, $content=null) {
	...
	$output = '<div class="sc_contact_form_field label_'.esc_attr($label_position).($class ? ' '.esc_attr($class) : '').($align && $align!='none' ? ' align'.esc_attr($align) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '') .'>'
		. ($label_position=='top' || $label_position=='left' ? $label : '')
		. ($type == 'textarea' 
			? '<textarea id="' . esc_attr($id) . '" name="' . esc_attr($name ? $name : $id) . '">' . esc_attr($value) . '</textarea>'
			: ($type=='button' 
				? '<div class="sc_contact_form_button"><div class="squareButton global ico"><a href="#" class="sc_contact_form_submit icon-comment-1">' . esc_html($value) . '</a></div></div>'
				: '<input type="'.($type ? $type : 'text').'" id="' . esc_attr($id) . '" name="' . esc_attr($name ? $name : $id) . '" value="' . esc_attr($value) . '" placeholder="' . esc_attr($placeholder) . '"' . (sc_param_is_on($checked) ? ' checked="checked"' : '') . '>'
				)
			)
		. ($label_position!='top' && $label_position!='left' ? $label : '')
		. '</div>';
...
function sc_content($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '')
		. ' class="sc_content main' . ($class ? ' ' . esc_attr($class) : '') . '"'
		. ($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
		. do_shortcode($content) 
		. '</div>';
...
function sc_countdown($atts, $content = null) {
	...
	if (date('Y-m-d H:i:s') < $date.' '.($time ? $time : '00:00:00')) {
		...
		$s = getStyleString($top, $right, $bottom, $left, $width, $height);
        return '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="sc_countdown_wrapper' . ($align && $align!='none' ? ' align' . esc_attr($align) : '') . (!empty($class) ? ' '.esc_attr($class) : '') .'"'.($s ? ' style="'.esc_attr($s).'"' : '').'><div class="sc_countdown" data-date="'.esc_attr($date).'" data-time="'.esc_attr($time).'"></div></div>';
	} else
		return '';
}
// ---------------------------------- [/trx_countdown] ---------------------------------------
...
function sc_dropcaps($atts, $content=null){
	...
	$content = do_shortcode($content);
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_dropcaps sc_dropcaps_style_' . esc_attr($style) . (!empty($class) ? ' '.esc_attr($class) : '') . '">'
...
function sc_emailer($atts, $content = null) {
    ...
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="sc_emailer inputSubmitAnimation' . ($align && $align!='none' ? ' sc_align' . esc_attr($align) : '') . (sc_param_is_on($open) ? ' sFocus rad4 opened' : ' radCircle') . (!empty($class) ? ' '.esc_attr($class) : '') . '"' . ($s ? ' style="'.esc_attr($s).'"' : '') . '>'
...
function sc_gallery_filter($prm1, $atts) {
	...
	$output = '<div id="sc_gallery_'.esc_attr($instance).'" class="sc_gallery columnsWrap">';
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		$output .= '
			<div class="columns1_'.esc_attr($columns).'">
				<div class="galleryPic">
					' . ($link=='file'
						...
						: '<div class="thumb">
								<a href="' . esc_url($url) . '">'.balanceTags($thumb).'</a>
							</div>') .'
...
function sc_google_map($atts, $content = null) {
    ...
    $output = '<div id="' . esc_attr($id) . '" class="sc_googlemap'. (!empty($class) ? ' '.esc_attr($class) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '')
...
function sc_icon($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$s2 = ($weight != '' ? 'font-weight:'. esc_attr($weight).';' : '')
		. ((int) $size > 0 ? 'font-size:'.esc_attr($size).'px;' : '')
		. ($color != '' ? 'color:'.esc_attr($color).';' : '')
		. ($bg_color != '' ? 'background-color:'.esc_attr($bg_color).';' : '')
	...
	;
    $output = $icon!=''
		? '<span' . ($id ? ' id="' . esc_attr($id) . '"' : '')
			.' class="sc_icon '.esc_attr($icon)
			.($background && $background!='none' ? ' sc_icon_'.esc_attr($background) : '')
			.($align ? ' sc_align'.esc_attr($align) : '')
			.(!empty($class) ? ' '.esc_attr($class) : '')
			.'"'
			.($s || $s2 ? ' style="'.($s ? 'display:block;' : '') . esc_attr($s) . esc_attr($s2) . '"' : '')
			.'></span>'
		: '';
...
function sc_image($atts, $content=null){	
	...
	if (trim($title) == '') {
        return empty($src) ? '' : ('<figure' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_image ' . ($align && $align!='none' ? ' sc_image_align_' . esc_attr($align) : '') . (!empty($shape) ? ' sc_image_shape_'.esc_attr($shape) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
			.'<img src="' . esc_url($src) . '" alt="" />'.(trim($title) || trim($icon) ? '' : '')
			. '</figure>');
	}
	else {
        $output = empty($src) ? '' : ('<figure' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_image ' . ($align && $align!='none' ? ' sc_image_align_' . esc_attr($align) : '') . (!empty($shape) ? ' sc_image_shape_'.esc_attr($shape) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
			.'<img src="' . esc_url($src) . '" alt="" />'.(trim($title) || trim($icon) ? '<figcaption><span'.($icon ? ' class="icon '.esc_attr($icon).'"' : '').'>' . esc_html($title) . '</span></figcaption>' : '')
			. '</figure>');
...
function sc_infobox($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_infobox sc_infobox_style_' . esc_attr($style) . (sc_param_is_on($closeable) ? ' sc_infobox_closeable' : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
			. do_shortcode($content) 
			. '</div>';
...
function sc_line($atts, $content=null){	
    ...
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_line' . ($style != '' ? ' sc_line_style_' . esc_attr($style) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'></div>';
...
function sc_list($atts, $content=null){	
    ...
    $output = '<' . ($style=='ol' ? 'ol' : 'ul') . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_list sc_list_style_' . esc_attr($style) . (!empty($class) ? ' '.esc_attr($class) : '') . '"' . ($s!='' ? ' style="'.esc_attr($s).'"' : '') . '>'
			. do_shortcode($content) 
			. '</' .($style=='ol' ? 'ol' : 'ul') . '>';
...
function sc_list_item($atts, $content=null) {
    ...
    $output =  '<li' . ($id ? ' id="' . esc_attr($id) . '"' : '')
		. ' class="sc_list_item' . ($icon!='' ? ' '.esc_attr($icon) : '')
		. (!empty($class) ? ' '.esc_attr($class) : '')
...
function sc_parallax($atts, $content=null){	
	...
	$s = getStyleString($top, '!'.$right, $bottom, '!'.$left, $width, $height)
		.($color !== '' && $overlay=='' ? 'background-color:' . esc_attr($color) . ';' : '')
		;
    $output = (sc_param_is_on($gap) ? sc_gap_start() : '')
		.'<div' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="sc_parallax' . ($video!='' ? ' sc_parallax_with_video' : '') . ($style!='' ? ' '.esc_attr($style) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
		. ($s!='' ? ' style="'.esc_attr($s).'"' : '')
		. ' data-parallax-speed="' . esc_attr($speed) . '"'
		. ' data-parallax-x-pos="' . esc_attr($image_x) . '"'
		. ' data-parallax-y-pos="' . esc_attr($image_y) . '"'
		. '>'
		. ($video!='' ? '<div class="sc_video_bg_wrapper"><video class="sc_video_bg" width="1280" height="720" data-width="1280" data-height="720" preload="metadata" autoplay="autoplay" loop="loop" src="'.esc_attr($video).'" data-ratio="'.esc_attr($video_ratio).'" data-frame="no"><source src="'.esc_url($video).'" type="video/'.esc_attr($ext).'"></source></video></div>' : '')
		. '<div class="sc_parallax_content" style="'.($image !== '' ? 'background-image:url(' . esc_attr($image) . '); background-position:'.esc_attr($image_x).' '.esc_attr($image_y).';' : '').'">'
		. ($overlay > 0 || $texture>0 ? '<div class="sc_parallax_overlay'.($texture!='' ? ' texture_bg_'.esc_attr($texture) : '').'" style="'.($overlay>0 ? 'background-color:rgba('.esc_attr($rgb['r']).','.esc_attr($rgb['g']).','.esc_attr($rgb['b']).','.min(1, max(0, $overlay)).');' : '').'">' : '')
		. do_shortcode($content)
...
function sc_popup($atts, $content=null){	
    ...
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_popup sc_popup_light mfp-with-anim mfp-hide' . ($class ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
			. do_shortcode($content) 
			. '</div>';
...
function sc_price($atts, $content=null){	
	...
	if (!empty($money)) {
		...
		if (count($m)==1) $m[1] = '';
		$output = '
			<div' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="sc_price_item'. (!empty($class) ? ' '.esc_attr($class) : '').'">
				<span class="sc_price_currency">'.esc_html($currency).'</span>
				<div class="sc_price_money">'.esc_html($m[0]).'</div>
				<div class="sc_price_info">
					<div class="sc_price_penny">'.esc_html($m[1]).'</div>
					<div class="sc_price_period">'.esc_html($period).'</div>
				</div>
			</div>
		';
	}
...
function sc_price_table($atts, $content=null){	
	...
	$THEMEREX_sc_price_table_counter = 0;
	$count = max(1, $count);
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_pricing_table columns_' . esc_attr($count) . ($align && $align!='none' ? ' align'.themerex_strtoproper($align) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"' . ($s!='' ? ' style="'.esc_attr($s).'"' : '') . '>'
			. do_shortcode($content)
		. '</div>';
...
function sc_price_item($atts, $content=null) {
    ...
    $output = '<div class="sc_pricing_columns sc_pricing_column_'.esc_attr($THEMEREX_sc_price_table_counter). (!empty($class) ? ' '.esc_attr($class) : '').'"><ul'.(sc_param_is_on($animation) ? ' class="columnsAnimate"' : '') . ($id ? ' id="' . esc_attr($id) . '"' : '') . '>'
		. do_shortcode($content) 
		. '</ul></div>';
...
function sc_price_data($atts, $content=null) {
	...
	if ($type=='price' && $money!='') {
		...
		if (count($m)==1) $m[1] = '';
		$content = '
			<div class="sc_price_item'. (!empty($class) ? ' '.esc_attr($class) : '').'">
				<span class="sc_price_currency">'.esc_html($currency).'</span>
				<div class="sc_price_money">'.esc_html($m[0]).'</div>
				<div class="sc_price_info">
					<div class="sc_price_penny">'.esc_html($m[1]).'</div>
		...
		$content = do_shortcode($content);
    $output = '<li' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_pricing_data' . ($type!='' ? ' sc_pricing_'.esc_attr($type) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '">' . balanceTags($content) . '</li>';
...
function sc_quote($atts, $content=null){	
	...
	$content = do_shortcode($content);
	if (themerex_substr($content, 0, 2)!='<p') $content = '<p>' . esc_html($content) . '</p>';
    $output = '<blockquote' . ($id ? ' id="' . esc_attr($id) . '"' : '') . esc_html($cite_param) . ' class="sc_quote'. (!empty($class) ? ' '.esc_attr($class) : '').'"' . ($s!='' ? ' style="'.esc_attr($s).'"' : '') . '>'
		. balanceTags($content)
		. ($title == '' ? '' : ('<p class="sc_quote_title">' . ($cite!='' ? '<a href="'.esc_url($cite).'">' : '') . esc_html($title) . ($cite!='' ? '</a>' : '') . '</p>'))
		.'</blockquote>';
...
function sc_section($atts, $content=null){	
	...
	$output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '')
		. ' class="sc_section' 
			...
			. (!empty($columns) && $columns!='none' ? ' columns'.esc_attr($columns) : '')
			. (sc_param_is_on($scroll) && !sc_param_is_off($controls) ? ' sc_scroll_controls sc_scroll_controls_'.esc_attr($dir).' sc_scroll_controls_type_'.esc_attr($controls) : '')
		. '"'
		. ($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
		. (sc_param_is_on($scroll) 
			? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_'.esc_attr($dir).' swiper-slider-container scroll-container"'
				. ' style="'.($dir=='vertical' ? 'height:'.($height != '' ? $height.'px' : "230px").';' : 'width:'.($width != '' ? $width.';' : "100%;")).'"'
				. '>'
				. '<div class="sc_scroll_wrapper swiper-wrapper">' 
				. '<div class="sc_scroll_slide swiper-slide">' 
			: '')
		. (sc_param_is_on($pan) ? '<div id="'.esc_attr($id).'_pan" class="sc_pan sc_pan_'.esc_attr($dir).'">' : '')
		. do_shortcode($content) 
...
function sc_skills($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left, $width, $height)
		.($align != '' && $align != 'none' ? 'float:' . esc_attr($align) . ';' : '');
	$content = do_shortcode($content);
	...
	return ($type!='arc' && $title!='' ? '<h2>'.esc_html($title).'</h2>' : '')
			. '<div' 
				. ($id ? ' id="' . esc_attr($id) . '"' : '')
				. ' class="sc_skills sc_skills_' . esc_attr($type) . ($type=='bar' ? ' sc_skills_'.esc_attr($dir) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
				. ($s!='' ? ' style="'.esc_attr($s).'"' : '')
				. ' data-type="'.esc_attr($type).'"'
				. ' data-subtitle="'.esc_attr($subtitle).'"'
				. ($type=='bar' ? ' data-dir="'.esc_attr($dir).'"' : '')
			. '>'
				. ($layout == 'columns' ? '<div class="columnsWrap sc_skills_'.esc_attr($layout).'">' : '')
				. ($type=='arc' 
					? ('<div class="sc_skills_legend">'.($title!='' ? '<h2>'.esc_html($title).'</h2>' : '').'<ul>'.($THEMEREX_sc_skills_legend).'</ul></div>'
...
function sc_skills_item($atts, $content=null) {
	...
	$title_block = '<div class="sc_skills_info">' . esc_html($title) . '</div>';
	$old_color = $color;
	...
	if ($THEMEREX_sc_skills_type=='arc') {
		if (empty($old_color)) {
			$rgb = Hex2RGB($color);
			$color = 'rgba('.esc_attr($rgb['r']).','.esc_attr($rgb['g']).','.esc_attr($rgb['b']).','.(1 - 0.1*($THEMEREX_sc_skills_counter-1)).')';
		}
		$THEMEREX_sc_skills_legend .= '<li style="background-color:'.esc_attr($color).'">' . esc_html($title) . '</li>';
		$THEMEREX_sc_skills_data .= '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="arc'. (!empty($class) ? ' '.esc_attr($class) : '').'"><input type="hidden" class="text" value="'.esc_attr($title).'" /><input type="hidden" class="percent" value="'.esc_attr($percent).'" /><input type="hidden" class="color" value="'.esc_attr($color).'" /></div>';
	} else {
		$output .= ($THEMEREX_sc_skills_columns > 0 ? '<div class="sc_skills_column columns1_'.esc_attr($THEMEREX_sc_skills_columns).'">' : '')
				. ($THEMEREX_sc_skills_type=='bar' && $THEMEREX_sc_skills_dir=='horizontal' ? $title_block : '')
				. '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_skills_item' . ($style ? ' sc_skills_style_'.esc_attr($style) : '') . ($THEMEREX_sc_skills_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_skills_counter == 1 ? ' first' : '') . '"'
					. ($THEMEREX_sc_skills_height !='' ? ' style="height: '.esc_attr($THEMEREX_sc_skills_height).';"' : '')
				. '>';
		if (in_array($THEMEREX_sc_skills_type, array('bar', 'counter'))) {
			$output .= '<div class="sc_skills_count'. (!empty($class) ? ' '.esc_attr($class) : '').'"' . ($THEMEREX_sc_skills_type=='bar' && $color ? ' style="background-color:' . esc_attr($color) . '; border-color:' . esc_attr($color) . '"' : '') . '>'
						. '<div class="sc_skills_total"'
							. ' data-start="'.esc_attr($start).'"'
							. ' data-stop="'.esc_attr($stop).'"'
							. ' data-step="'.esc_attr($step).'"'
							. ' data-max="'.esc_attr($THEMEREX_sc_skills_max).'"'
			...
			$output .= '<canvas id="'.esc_attr($id).'"></canvas>'
				. '<div class="sc_skills_total'. (!empty($class) ? ' '.esc_attr($class) : '').'"'
					. ' data-start="'.esc_attr($start).'"'
					. ' data-stop="'.esc_attr($stop).'"'
					. ' data-step="'.esc_attr($step).'"'
...
function sc_slider($atts, $content=null){	
	...
	$output = ($border!='none' 
				? '<div class="sc_border sc_border_'.esc_attr($border).($align!='' && $align!='none' ? ' sc_align'.esc_attr($align) : '').'"'.(esc_attr($ms.$hs) ? ' style="'.esc_attr($ms.$hs).'"' : '').'>'
				: '')
			. (in_array($pagination, array('full', 'over')) 
				? '<div class="sc_slider_pagination_area sc_slider_pagination_'.esc_attr($pagination).'"'
					.(esc_attr($ms.$hs) ? ' style="'.esc_attr($ms.$hs).'"' : '') .'>'
				: '')
			. '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '')
			. ' class="sc_slider'
				. (!empty($class) ? ' '.esc_attr($class) : '')
				. ' sc_slider_' . esc_attr($engine)
				...
				. ($border=='none' && $align!='' && $align!='none' ? ' sc_align'.esc_attr($align) : '')
	...
	if ($engine=='revo') {
			...
			while ( $query->have_posts() ) { 
				...
				if (in_array($pagination, array('full', 'over'))) {
					...
					if (($post_tags_list = get_the_tags()) != 0) {
						...
						foreach ($post_tags_list as $tag) {
							$tag_number++;
							$post_tags_links .= '<span class="slide_tag">' . esc_html($tag->name) . ($tag_number==count($post_tags_list) ? '' : ',') . '</span> ';
						}
					}
					$pagination_items .= '<li'.(empty($pagination_items) ? ' class="'.($engine=='chop' ? 'cs-active-pagination' : 'active').'"' : '').'>' 
						.'<div class="slide_pager">'
						.'<div class="slide_date">'.esc_html($post_date).'</div>'
						.'<div class="slide_info">'
						.'<h4 class="slide_title">'.esc_html($post_title).'</h4>'
						.'<div class="slide_tags">'.esc_html($post_tags_links).'</div>'
						.'</div>'
						.'</div>'
						.'</li>'
						;
				...
				$output .= '<li' 
					...
					. ' style="'
						...
						. ($imageAsBackground ? 'background-image:url(' . esc_url($post_attachment) . ');' : '')
						. esc_attr($ws)
						. esc_attr($hs)
						. '"'
					. '>' 
				...
				if (!sc_param_is_off($titles)) {
					...
					$caption .= ($engine=='chop' ? '<div class="sc_slider_info_item">' : '') . '<div class="sc_slider_info' . ($titles=='fixed' ? ' sc_slider_info_fixed' : '') . ($engine=='swiper' ? ' content-slide' : '') . '"'.($post_bg!='' ? ' style="'.esc_attr($post_bg).'"' : '').'>';
					...
					if (get_custom_option("slider_info_category")=='yes') { // || empty($cat)) {
						...
						if ($post_category!='') {
							$caption .= '<div class="sc_slider_category"'.(themerex_substr($post_accent_color, 0, 1)=='#' ? ' style="background-color: '.esc_attr($post_accent_color).'"' : '').'><a href="'.esc_url($post_category_link).'">'.esc_html($post_category).'</a></div>';
						}
					}
					$output_reviews = '';
					...
					$caption .= '<h2 class="sc_slider_subtitle"><a href="'.esc_url($post_link).'">'.esc_html($post_title).'</a></h2>';
					if (get_custom_option("slider_info_category")!='yes') $caption .= $output_reviews;
					if ($descriptions > 0) {
		...
		if ($engine=='chop') {
			$output .= '
				<div class="sc_slider_info_slides">'.esc_html($caption).'</div>
	...
	if (!empty($output)) {
		...
		if ($pagination_items) {
			$output .= '
				<div class="flex-control-nav manual"'.($hs ? ' style="'.esc_attr($hs).'"' : '').'>
					<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical swiper-slider-container scroll-container"'.($hs ? ' style="'.esc_attr($hs).'"' : '').'>
						<div class="sc_scroll_wrapper swiper-wrapper">
							<div class="sc_scroll_slide swiper-slide">
								<ul>'.balanceTags($pagination_items).'</ul>
							</div>
						</div>
...
function sc_slider_item($atts, $content=null) {
    ...
    $output = empty($src) ? '' : ('<li' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="' . esc_attr($THEMEREX_sc_slider_engine).'-slide' . (!empty($class) ? ' '.esc_attr($class) : '') . '"'
		. ' style="background-image:url(' . esc_url($src) . ');'
			. (!empty($THEMEREX_sc_slider_width) ? 'width:' . esc_attr($THEMEREX_sc_slider_width) . ';' : '')
			. (!empty($THEMEREX_sc_slider_height) ? 'height:' . esc_attr($THEMEREX_sc_slider_height) . ';' : '')
		.'">' 
...
function sc_table($atts, $content=null){	
    ...
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_table sc_table_style_' . max(1, min(4, $style)) . ' sc_table_size_' . esc_attr($size) . (!empty($align) ? ' sc_table_align_'.esc_attr($align) : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '') .'>'
			. do_shortcode($content) 
			. '</div>';
...
function sc_tabs($atts, $content = null) {
	...
	$tabs_output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_tabs sc_tabs_style_'.esc_attr($style). (!empty($class) ? ' '.esc_attr($class) : '') . '"'.($s!='' ? ' style="'.esc_attr($s).'"' : '') .' data-active="' . ($initial-1) . '">'
	...
	for ($i = 0; $i < count($THEMEREX_sc_tab_titles); $i++) {
		...
		$titles_output .= '<li class="'.join(' ', $classes).'"><a href="#'.esc_attr($THEMEREX_sc_tab_titles[$i]['id']).'" class="theme_button" id="'.esc_attr($THEMEREX_sc_tab_titles[$i]['id']).'_tab">' . balanceTags($THEMEREX_sc_tab_titles[$i]['title']) . '</a></li>';
	}
...
function sc_tab($atts, $content = null) {
    ...
    $output = '<div id="' . esc_attr($id) . '" class="sc_tabs_content' . ($THEMEREX_sc_tab_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_tab_counter == 1 ? ' first' : '') . (!empty($class) ? ' '.esc_attr($class) : '') . '">'
		. (sc_param_is_on($THEMEREX_sc_tab_scroll) ? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.($THEMEREX_sc_tab_height != '' ? $THEMEREX_sc_tab_height : '230px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' : '')
		. do_shortcode($content) 
		. (sc_param_is_on($THEMEREX_sc_tab_scroll) ? '</div></div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_vertical '.esc_attr($id).'_scroll_bar"></div></div>' : '')
		. '</div>';
...
function sc_team($atts, $content=null){	
	...
	$THEMEREX_sc_team_counter = 0;
	$content = do_shortcode($content);
    $output = '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_team'.(!empty($class) ? ' '.esc_attr($class) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '') .'>'
...
function sc_team_item($atts, $content=null) {
	...
	if (!empty($user) && $user!='none' && ($user_obj = get_user_by('login', $user)) != false) {
		...
		for ($i=0; $i<count($allowed); $i++) {
			...
			if (!empty($s[1])) {	// && array_key_exists($s[0], $THEMEREX_user_social_list)) {
				...
				$socials .= '<li><a href="' . esc_url($s[1]) . '" class="social_icons social_' . esc_attr($s[0]) . ' ' . esc_attr($s[0]) . '" target="_blank" style="background-image: url('.esc_url($img).');">'
						. '<span style="background-image: url('.esc_url($img).');"></span>'
						. '</a></li>';
			}
		}
	}
	...
	if (!empty($name) && !empty($position)) {
        $output = '<div class="columns1_'.esc_attr($THEMEREX_sc_team_count) . (!empty($class) ? ' '.esc_attr($class) : '').'">'
				. '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_team_item sc_team_item_' . esc_attr($THEMEREX_sc_team_counter) . ($THEMEREX_sc_team_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_team_counter == 1 ? ' first' : '') . '">'
					. '<div class="sc_team_item_avatar">'
						. balanceTags($photo)
						. '<div class="sc_team_item_description">' . esc_html($descr) . '</div>'
					. '</div>'
					. '<div class="sc_team_item_info">'
					. '<h3 class="sc_team_item_title">' . esc_html($name) . '</h3>'
					. '<div class="sc_team_item_position theme_accent2">' . esc_attr($position) . '</div>'
					. (!empty($socials) ? '<ul class="sc_team_item_socials">' . balanceTags($socials) . '</ul>' : '')
					. '</div>'
				. '</div>'
			. '</div>';
	}
...
function sc_testimonials($atts, $content=null){	
    ...
    $output = ($title && $style>1 ? '<h2 class="sc_testimonials_title">'.esc_html($title).'</h2>' : '')
		. '<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_testimonials sc_testimonials_style_'.esc_attr($style).($title && $style==1 || $controls=='top' ? ' sc_testimonials_padding' : '').(sc_param_is_off($controls) ? '' : ' sc_testimonials_controls_'.esc_attr($controls)) . (!empty($class) ? ' '.esc_attr($class) : '') .'"' . ($s!='' ? ' style="'.esc_attr($s).'"' : '') . '>'
			. ($title && $style==1 ? '<h2 class="sc_testimonials_title">'.esc_html($title).'</h2>' : '')
			. ($THEMEREX_sc_testimonials_count>1 ? '<div class="sc_slider sc_slider_swiper'.(sc_param_is_off($controls) ? '' : ' sc_slider_controls sc_slider_controls_'.esc_html($controls)).' sc_slider_nopagination sc_slider_autoheight swiper-slider-container"'.($d ? ' style="'.esc_attr($d).'"' : '') . ((int) $interval > 0 ? ' data-interval="'.esc_attr($interval).'"' : '') . '>' : '')
...
function sc_testimonials_item($atts, $content=null){
	...
	if (empty($photo)) {
		...
		$photo = getResizedImageTag($photo, 100, 100);
	}
	if (empty($photo)) $photo = '<img src="'.themerex_get_file_url('/images/no-ava.png').'" alt="">';
    $output = '<li' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_testimonials_item swiper-slide'. (!empty($class) ? ' '.esc_attr($class) : '').'" style="'.(!empty($THEMEREX_sc_testimonials_width) ? 'width:' . esc_attr($THEMEREX_sc_testimonials_width) . ';' : '').(!empty($THEMEREX_sc_testimonials_height) ? 'height:' . esc_attr($THEMEREX_sc_testimonials_height) . ';' : '').'">'
				. '<div class="sc_testimonials_item_content">'
					. '<div class="sc_testimonials_item_quote"><div class="sc_testimonials_item_text">'.do_shortcode($content).'</div></div>'
					. '<div class="sc_testimonials_item_author">'
						. '<div class="sc_testimonials_item_avatar">'.balanceTags($photo).'</div>'
						. '<div class="sc_testimonials_item_name">'.esc_html($name).'</div>'
						. '<div class="sc_testimonials_item_position">'.esc_html($position).'</div>'
					. '</div>'
...
function sc_title($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left, $width)
		.($align && $align!='inherit' ? 'text-align:' . esc_attr($align) .';' : '')
		.($color ? 'color:' . esc_attr($color) .';' : '')
		.($weight && $weight!='inherit' ? 'font-weight:' . esc_attr($weight) .';' : '');
	...
	$pic = $style!='iconed' 
		? '' 
		: ''
			.(sc_param_is_on($link) ? '<a class="sc_title_linked" href="' . ($url != '' ? esc_url($url) : '#') . '">' : '')
			.'<'
			.($size!='small' ? 'div' : 'span').' class="sc_title_'.($icon ? 'icon' : 'image').' sc_title_'.esc_attr($position).' sc_size_'.esc_attr($size)
			.($icon!='' && $icon!='none' ? ' '.esc_attr($icon) : '')
			.(!empty($background) && $background!='none' ? ' sc_title_bg sc_bg_'.esc_attr($background) : '')
			.'"'
			.(!empty($background) && $background!='none' && $bg_color!='' ? ' style="background-color:'.esc_attr($bg_color).'"' : '').'>'
			.($picture ? '<img src="'.esc_url($picture).'" alt="" />' : '')
			.(empty($picture) && $image && $image!='none' ? '<img src="'.(themerex_strpos($image, 'http:')!==false ? $image : themerex_get_file_url('/images/icons/'.esc_attr($image).'.png')).'" alt="" />' : '')
			.'</'.($size!='small' ? 'div' : 'span').'>';
    $output = ($size!='small' ? $pic : '')
		. (($style == 'iconed') ? '' : (sc_param_is_on($link) ? '<a class="sc_title_linked" href="' . ($url != '' ? esc_url($url) : '#') . '">' : ''))
		. '<h' . esc_attr($type) . ($id ? ' id="' . esc_attr($id) . '"' : '')
		. ' class="sc_title sc_title_'.esc_attr($style). (!empty($class) ? ' '.esc_attr($class) : '').'"'
		. ($s!='' ? ' style="'.esc_attr($s).'"' : '')
		. '>'
		. ($size=='small' ? $pic : '')
...
function sc_tooltip($atts, $content=null){	
    ...
    $output = '<span' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_tooltip_parent'. (!empty($class) ? ' '.esc_attr($class) : '').'">' . do_shortcode($content) . '<span class="sc_tooltip">' . esc_html($title) . '</span></span>';
...
add_shortcode('trx_social', 'sc_social');

/*
[trx_social id="unique_id" class=""][/trx_social]
*/
function sc_social($atts, $content=null){
	...
	foreach ($socials as $s) {
		...
		$out .= '<li><a class="social_icons social_' . esc_attr($snn) . '" style="background-image: url(' . esc_url($socn) . ');" target="_blank" href="' . esc_url($surl) . '"><span style="background-image: url(' . esc_url($socn) . ');"></span></a></li>';
	}

	return '<ul '
	. ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_social' . (!empty($class) ? ' ' . esc_attr($class) : '') . '">'
	. balanceTags($out)
	. '</ul>';
}
// ---------------------------------- [/trx_tooltip] ---------------------------------------
...
function sc_twitter($atts, $content=null){
	...
	if (!empty($twitter_consumer_key) && !empty($twitter_consumer_secret) && !empty($twitter_token_key) && !empty($twitter_token_secret)) {
		...
		if ($data && isset($data[0]['text'])) {
			$output = '
				<div' . ($id ? ' id="' . esc_attr($id) . '"' : '').' class="sc_twitter twitBlock'.($class ? ' '.esc_attr($class) : '').'"'. ($s!='' ? ' style="'.esc_attr($s).'"' : '').'>
			...
			foreach ($data as $tweet) {
				...
				$output .= '<li class="sc_twitter_item swiper-slide' . ($cnt==$twitter_count-1 ? ' last' : '') . '">'
						...
						. '<a href="https://twitter.com/' . esc_attr($twitter_username) . '" class="twitAuthor" target="_blank">@' . htmlspecialchars($tweet['user']['screen_name']) . '</a> '
...
function sc_zoom($atts, $content=null){	
    ...
    $output = empty($src) ? '' : ((!sc_param_is_off($border) ? '<div class="sc_border sc_border_'.esc_attr($border).'">' : '')
				.'<div' . ($id ? ' id="' . esc_attr($id) . '"' : '') . ' class="sc_zoom'. (!empty($class) ? ' '.esc_attr($class) : '').'"'.($s!='' ? ' style="'.esc_attr($s).'"' : '').'>'
					.'<img src="' . esc_url($src) . '"'.($height != '' ? ' style="height:' . esc_attr($height) . ';"' : '').' data-zoom-image="'.esc_attr($over).'" alt="" />'
				. '</div>'
