...
$top_panel_opacity = get_custom_option('top_panel_opacity');
...
<head>
	...
	if (!$favicon) {
		...
		if ( file_exists(themerex_get_file_dir('/skins/'.esc_attr($skin).'/images/favicon.ico')) )
			$favicon = themerex_get_file_url('/skins/'.esc_attr($skin).'/images/favicon.ico');
...
<body <?php 
	...
	if ($body_style=='boxed' || get_custom_option('load_bg_image')=='always') {
		...
		if ($customizer && ($img = (int) getValueGPC('bg_image', 0)) > 0)
			$class = 'bg_image_'.esc_attr($img);
		else if ($customizer && ($img = (int) getValueGPC('bg_pattern', 0)) > 0)
			$class = 'bg_pattern_'.esc_attr($img);
		else if ($customizer && ($img = getValueGPC('bg_color', '')) != '')
			$style = 'background-color: '.esc_attr($img).';';
		else {
			if (($img = get_custom_option('bg_custom_image')) != '')
				$style = 'background: url('.esc_url($img).') ' . str_replace('_', ' ', get_custom_option('bg_custom_image_position')) . ' no-repeat fixed;';
	...
	body_class('themerex_body ' . ($body_style) . ' top_panel_' . ($show_top_panel) . ' top_panel_opacity_' . ($top_panel_opacity) . ' theme_skin_' . get_custom_option('theme_skin')
			...
			<div class="sc_scroll sc_scroll_vertical swiper-slider-container scroll-container" id="sidemenu_scroll">
				<div class="sc_scroll_wrapper swiper-wrapper">
					<div class="sc_scroll_slide swiper-slide">
						...
						<div class="logo<?php echo ($logo_text ? ' with_text' : ''); ?>"><a href="<?php echo home_url(); ?>"><?php echo ($logo_side ? '<img src="'.esc_url($logo_side).'" class="logo_side" alt="">' : ''); ?><?php echo ($logo_text ? '<span class="logo_text">'.apply_filters('theme_logo_text', $logo_text, 'sidemenu').'</span>' : ''); ?><?php echo ($logo_slogan ? '<span class="logo_slogan">'.esc_html($logo_slogan).'</span>' : ''); ?></a></div>
		...
		<div class="boxedWrap">
			<?php 
				if (($header_custom_image = get_header_image()) != '') {
					$header_style = ' style = "background-image: url('.esc_url($header_custom_image).');"';
				} else {
					$header_style = '';
				}
			?>
					...
					<div class="mainmenu_area">
						<div class="main with_logo_<?php echo get_custom_option('logo_align'); ?>">
							<div class="logo logo_<?php echo get_custom_option('logo_align'); ?><?php echo ($logo_text ? ' with_text' : ''); ?>">
								<a href="<?php echo home_url(); ?>"><?php echo ($logo_image ? '<img src="'.esc_url($logo_image).'" class="logo_main" alt=""><img src="'.esc_url($logo_fixed).'" class="logo_fixed" alt="">' : ''); ?><?php echo ($logo_text ? '<span class="logo_text">'.apply_filters('theme_logo_text', $logo_text, 'mainmenu').'</span>' : ''); ?><?php echo ($logo_slogan ? '<span class="logo_slogan">'.esc_html($logo_slogan).'</span>' : ''); ?></a>
							</div>
