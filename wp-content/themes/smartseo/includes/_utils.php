...
if (!function_exists('getOptionsFromArray')) {
	function getOptionsFromArray($arr, $cur) {
		$rezList = "";
		foreach ($arr as $k=>$v) {
			$rezList .= "\n".'<option value="'.esc_attr($k).'"'.($cur==$k ? ' selected="selected">' : '>').htmlspecialchars($v).'</option>';
		}
		return $rezList;
	}
}
...
if (!function_exists('getStyleString')) {
	function getStyleString($top='',$right='',$bottom='',$left='',$width='',$height='') {
		...
		foreach ($top as $k=>$v) {
			...
			if ($v != '') $output .= ($k=='width' ? 'width' : ($k=='height' ? 'height' : 'margin-'.esc_attr($k))) . ':' . getStyleValue($v) . ($imp=='!' ? ' !important' : '') . ';';
		}
		return $output;
	}
}
	...
	function twitter_prepare_text($tweet) {
		...
		if (!empty($tweet['entities']['urls']) && count($tweet['entities']['urls']) > 0) {
			foreach ($tweet['entities']['urls'] as $url) {
				$text = str_replace($url['url'], '<a href="'.esc_url($url['expanded_url']).'" target="_blank">' . esc_html($url['display_url']) . '</a>', $text);
			}
		}
