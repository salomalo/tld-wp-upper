			...
			if (!$list) {
				...
				if (count($terms) > 0) {
					...
					foreach ($terms as $slug=>$name) {
						$list .= '<option value='. esc_attr($slug) . (isset($_REQUEST[$tax_slug]) && $_REQUEST[$tax_slug] == $slug || (isset($_REQUEST['taxonomy']) && $_REQUEST['taxonomy'] == $tax_slug && isset($_REQUEST['term']) && $_REQUEST['term'] == $slug) ? ' selected="selected"' : '') . '>' . esc_url($name) . '</option>';
					}
					$list .=  "</select>";
				}
			}
	...
	function showBreadcrumbs($args=array()) {
		...
		if ( !in_array($type, array('home', 'frontpage')) ) {
						...
						if ($cat) {
							if (!in_array($cat['term_id'], $ex_cats)) {
								...
								if ($level > $args['max_levels'])
									...
									$rez2 = '<a class="cat_post" href="' . esc_url($cat_link) . '">' . getShortString($cat['name'], $args['truncate_title'], $args['truncate_add']) . '</a>' . (!empty($rez2) ? $args['delimiter'] : '') . ($rez2);
							}
						} else {
					...
					if ($cat) {
						if (!in_array($cat['term_id'], $ex_cats)) {
							...
							if ($level > $args['max_levels'])
								...
								$rez2 = '<a class="cat_parent" href="' . esc_url($cat_link) . '">' . getShortString($cat['name'], $args['truncate_title'], $args['truncate_add']) . '</a>' . (!empty($rez2) ? $args['delimiter'] : '') . ($rez2);
						}
						$parent = $cat['parent'];
					}
				}
			...
			if (themerex_strpos($type, 'woocommerce')!==false && ($shop_id=get_option('woocommerce_shop_page_id'))>0 && !in_array(themerex_strtolower($title), array(themerex_strtolower($shop_title=getPostTitle($shop_id))))) {
				$rez_all = '<a class="all" href="' . get_permalink($shop_id) . '">' . esc_html($shop_title) . '</a>' . (!empty($rez_all) ? $args['delimiter'] : '') . ($rez_all);
			}
			...
			if ($type == 'tribe_day' && is_object($post)) {
				...
				if (is_day() || is_month())
					$rez3 .= (!empty($rez3) ? $args['delimiter'] : '') . '<a class="cat_parent" href="' . get_year_link( $year ) . '">' . esc_html($year) . '</a>';
				if (is_day())
			...
			if (!is_front_page()) {	// && !is_home()
				$rez .= (isset($args['home']) && $args['home']!='' ? '<a class="home" href="' . ($args['home_url'] ? $args['home_url'] : home_url()) . '">' . esc_html($args['home']) . '</a>' . balanceTags($args['delimiter']) : '')
...
if (!function_exists('getBlogType')) {
	...
	global $wp_query;
		...
		if (  $page == 'blog.php')			// || is_page_template( 'blog.php' ) )
			...
			return $page;
		else if ( themerex_strpos($page, 'tribe')!==false )					// Tribe__Events__Main
			return $page;
...
if (!function_exists('getThumbSizes')) {
	function getThumbSizes($opt) {
		...
		$thumb_sizes = array(
			...
			'related'           => array('w' => 400, 'h' => $opt['thumb_crop'] ? 277 : null, 'h_crop' => 277),	// 400 x 225
			...
			'image_large'       => array('w' => 400, 'h' => $opt['thumb_crop'] ? 277 : null, 'h_crop' => 277),	// 400 x 225
			'masonry2'          => array('w' => 714, 'h' => $opt['thumb_crop'] ? 402 : null, 'h_crop' => 402),	// 550 x 309
			'masonry3'          => array('w' => 400, 'h' => $opt['thumb_crop'] ? 277 : null, 'h_crop' => 277),	// 350 x 197
			...
			'classic3'          => array('w' => 400, 'h' => $opt['thumb_crop'] ? 277 : null, 'h_crop' => 277),	// 350 x 197
				...
				if ($gallery_items_in_bg) {
					...
					$gallery_text .= '<li'
						...
						. ($zoom ? '<a href="'. esc_url($photo) . '">'.esc_html($photo_min).'</a>'
								 : (!empty($link) ? '<a href="'. $link . '">'.esc_html($photo_min).'</a>' : $photo_min))
						. '</li>';
				}
			}
			$gallery_text .= '</ul>';
	...
	function substituteAudio($post_text) {
		...
		while (($pos_start = themerex_strpos($post_text, themerex_substr($tag, 0, -1).' ', $pos_start+1))!==false) {
			...
			if ($src != '') {
				...
				$container = '<div'.($id ? ' id="'.esc_attr($id).'"' : '').' class="audio_container' . ($tag_a ? ' align'.esc_attr($tag_a) : '') . '"' . ($tag_s || $tag_w || $tag_h ? ' style="'.($tag_w!='' ? 'width:' . $tag_w . (themerex_substr($tag_w, -1)!='%' ? 'px' : '') . ';' : '').($tag_h!='' ? 'height:' . $tag_h . 'px;' : '') . $tag_s . '"' : '') . '>';
...
if (!function_exists('getVideoFrame')) {
	function getVideoFrame($video, $image='', $title=false, $style='') {
		...
		$html = '<div class="sc_video_player"' . ' data-width="'.esc_attr($tag_w).'" data-height="'.esc_attr($tag_h).'"'.($style ? ' style="' . $style . '"' : '') . '>'
...
if (!function_exists('showShareSocialLinks')) {
	function showShareSocialLinks($args) {
		...
		$output = $args['style']=='block'
			? '<div class="post_info post_info_bottom"><div class="share-social share-dir-' . $args['direction'] . '">' . ($args['caption']!='' ? '<span class="share-caption">'.esc_html($args['caption']).'</span>' : '')
		...
		foreach ($args['share'] as $s => $data) {
			...
			$output .= (in_array($args['style'], array('list', 'drop')) ? '<li>' : '') 
				...
				. '>' 
				. ($args['use_icons'] ? '' : '<img src="'.esc_url($data['icon']).'" alt="' . $s . '">')
			...
			if (array_key_exists($s, $THEMEREX_user_social_list)) {
				...
				if ($link) {
					...
					$output .= $args['before']
						...
						. ($args['style']=='bg' ? ' style="background-image: url('.esc_url($img).');"' : '')
						. '>'
						. ($args['style']=='icons' ? '<span class="icon-' . $s . '"></span>' : ($args['style']=='images' ? '<img src="'.esc_url($img).'" alt="" />' : '<span style="background-image: url('.esc_url($img).');"></span>'))
						. '</a>'
						. $args['after'];
				}
			}
...
if (!function_exists('getResizedImageTag')) {
	function getResizedImageTag( $post, $w=null, $h=null, $c=null, $u=true, $find_thumb=false, $itemprop=false ) {
		...
		return $url!='' ? ('<img class="wp-post-image"' . ($w ? ' width="'.esc_attr($w).'"' : '') . ($h ? ' height="' . $h . '"' : '') . ' alt="' . $alt . '" src="' . $url . '"' . ($itemprop ? ' itemprop="image"' : '') . '>') : '';
	}
}
