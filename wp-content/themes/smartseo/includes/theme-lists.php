	...
	function getCustomFontsList($prepend_inherit=false) {
		...
		if ( is_dir($dir) ) {
			...
			if ( $hdir ) {
				while (($file = readdir( $hdir ) ) !== false ) {
					$pi = pathinfo( $dir . '/' . ($file) );
					if ( substr($file, 0, 1) == '.' || ! is_dir( $dir . '/' . esc_html($file) ) )
						continue;
					$css = file_exists( $dir . '/' . esc_html($file) . '/' . esc_html($file) . '.css' )
						? themerex_get_folder_url("/css/font-face/".esc_html($file).'/'.esc_html($file).'.css')
						: (file_exists( $dir . '/' . esc_html($file) . '/stylesheet.css' )
							? themerex_get_folder_url("/css/font-face/".esc_html($file).'/stylesheet.css')
							: '');
					if ($css != '')
