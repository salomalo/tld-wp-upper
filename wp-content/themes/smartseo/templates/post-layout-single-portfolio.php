...
'	<section class="' . join(' ', get_post_class('itemPage post post_format_'.esc_attr($post_data['post_format']).' post'.esc_attr($opt['post_class']) . (get_custom_option("show_post_author") == 'yes' || get_custom_option("show_post_related") == 'yes' || get_custom_option("show_post_comments") == 'yes' ? ' hrShadow' : ' no_margin'))) . '">',
