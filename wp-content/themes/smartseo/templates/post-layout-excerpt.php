...
$post_info = '
	<div class="post_info infoPost">
		' . __('Posted', 'themerex') . ' <a href="' . esc_url($post_data['post_link']) . '" class="post_date">' . esc_html($post_data['post_date']) . '</a>
		<span class="separator">|</span>
		' . __('by', 'themerex') . ' <a href="' . esc_url($post_data['post_author_url']) . '" class="post_author">' . esc_html($post_data['post_author']) . '</a>
...
<div class="<?php echo 'post'.esc_attr($opt['post_class']) . ($opt['number'] < $opt['posts_on_page'] ? ' hrShadow marginBottom' : '') . ($opt['number']%2==0 ? ' even' : ' odd') . ($opt['number']==0 ? ' first' : '') . ($opt['number']==$opt['posts_on_page']? ' last' : '') . ($opt['add_view_more'] ? ' viewmore' : ''); ?>">
<?php } else { 
	$post_classes = get_post_class('post_format_'.esc_attr($post_data['post_format']).($post_data['post_type']!='post' ? ' post' : '').' post'.esc_attr($opt['post_class']).' hrShadow'.($opt['number']%2==0 ? ' even' : ' odd') . ($opt['number']==0 ? ' first' : '') . ($opt['number']==$opt['posts_on_page']? ' last' : '') . ($opt['add_view_more'] ? ' viewmore' : ''));
?>
			...
			<div class="sc_section columns<?php echo ($opt['location']=='center' && $opt['sidebar'] ? '2_3' : '1_2'); ?> post_thumb thumb">
				...
				if ($post_data['post_video']) {
					...
					if ($post_data['post_format']=='link' && $post_data['post_url']!='')
						...
						echo '<a href="'.esc_url($post_data['post_link']).'">'.balanceTags($post_data['post_thumb']).'</a>';
					else
