...
if ($opt['style'] == 'list') {
	...
	$thumb = $post_data['post_thumb'] && themerex_strpos($opt['style'], 'image')!==false
				? ('<div class="thumb">' . ($post_data['post_link']!='' ? '<a href="'.esc_url($post_data['post_link']).'">'.balanceTags($post_data['post_thumb']).'</a>' : balanceTags($post_data['post_thumb'])) . '</div>')
				: '';
	...
	$info = sc_param_is_on($opt['info']) ? '<div class="sc_blogger_info">'
		. (themerex_strpos($opt['style'], 'image')!==false || themerex_strpos($opt['style'], 'accordion')!==false 
			? '<div class="squareButton light ico sc_blogger_more"><a class="icon-link" title="" href="'.esc_url($post_data['post_link']).'">'.($opt['readmore'] ? $opt['readmore'] : __('More', 'themerex')).'</a></div><div class="sc_blogger_author">' . __('Posted by', 'themerex')
			: __('by', 'themerex'))
