<div class="swpRightPos">
	...
	<div class="sc_tabs" data-active="<?php echo esc_attr($tab); ?>">
		...
		if (get_theme_option('show_theme_customizer') == 'yes') {
			...
			$theme_color2 = apply_filters('theme_skin_get_theme_color2', get_custom_option('theme_color2'));
			$theme_color3 = apply_filters('theme_skin_get_theme_color3', get_custom_option('theme_color3'));
								...
								<div class="co_options">
									<form name="co_form">
										...
										<div class="co_form_row">
											...
											<div class="co_form_subrow">
												...
												<span class="co_label one_row"><?php _e('Color 1:', 'themerex'); ?></span>
												<div id="co_theme_color" class="iColorPicker"></div>
												<input type="hidden" name="co_theme_color2" value="<?php echo esc_attr($theme_color2); ?>" />
												<span class="co_label one_row"><?php _e('Color 2:', 'themerex'); ?></span>
												<div id="co_theme_color2" class="iColorPicker"></div>
												<input type="hidden" name="co_theme_color3" value="<?php echo esc_attr($theme_color3); ?>" />
												<span class="co_label one_row"><?php _e('Color 3:', 'themerex'); ?></span>
										...
										<div class="co_form_row">
											...
											<div id="co_bg_pattern_list">
												<?php for ($i=0; $i<=9; $i++) { ?>
												<a href="#" id="pattern_<?php echo esc_attr($i); ?>" class="co_pattern_wrapper<?php echo esc_attr($bg_pattern==$i ? ' current' : '') ; ?>"><img src="<?php echo themerex_get_file_url('/images/bg/pattern_'.esc_attr($i).'_thumb2.png'); ?>" width="22" height="22" alt="" /></a>
												<?php } ?>
											</div>
										</div>
										<div class="co_form_row">
											...
											<div id="co_bg_images_list">
												<?php for ($i=1; $i<=6; $i++) { ?>
												<a href="#" id="image_<?php echo esc_attr($i); ?>" class="co_image_wrapper<?php echo esc_attr($bg_image==$i ? ' current' : '') ; ?>"><img src="<?php echo themerex_get_file_url('/images/bg/image_'.esc_attr($i).'_thumb2.jpg'); ?>" width="48" height="28" alt="" /></a>
												<?php } ?>
											</div>
										</div>
									</form>
									<script type="text/javascript">
										jQuery(document).ready(function(){
											...
											jQuery('#co_theme_color2').css('backgroundColor', '<?php echo esc_attr($theme_color2); ?>');
											jQuery('#co_theme_color3').css('backgroundColor', '<?php echo esc_attr($theme_color3); ?>');
				...
				<div id="sidebar_panel" class="widget_area sidebar_panel sidebar" role="complementary">
					<div id="sidebar_panel_scroll" class="sc_scroll sc_scroll_vertical swiper-slider-container scroll-container">
						<div class="sc_scroll_wrapper swiper-wrapper">
							...
							if ( is_active_sidebar( get_custom_option( 'sidebar_panel' ) ) ) {
				...
				<div class="sc_scroll_wrapper swiper-wrapper">
					<div class="sc_scroll_slide swiper-slide swiper-no-swiping">
						...
						<ol class="listBookmarks">
							...
							if (!empty($list)) {
								foreach ($list as $bm) {
									echo '<li><a href="'.esc_url($bm['url']).'">'.esc_html($bm['title']).'</a><a href="#" class="delBookmark icon-cancel"></a></li>';
								}
							}
							?>
						</ol>
