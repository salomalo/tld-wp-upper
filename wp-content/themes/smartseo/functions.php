...
if ( !function_exists( 'themerex_theme_setup' ) ) {
	function themerex_theme_setup() {
		...
		add_image_size( 'classic3',  400, 277, true );
			...
			if (!is_array($widgets) || count($widgets) == 0) {
				...
				if (is_array($sidebars) && count($sidebars) > 0) {
					foreach ($sidebars as $i => $sb) {
						...
						$widgets['custom-sidebar-'.esc_attr($i)]  = $sb;
					}
				}
			}
			if (count($widgets) > 0) {
	...
	function themerex_scripts() {
		...
		if (get_custom_option('typography_custom')=='yes') {
			...
			foreach ($selectors as $s) {
				$font = get_custom_option('typography_'.esc_attr($s).'_font');
...
function themerex_init_template() {
	...
	if ( file_exists(themerex_get_file_dir('/skins/'.esc_attr($skin).'/'.esc_attr($skin).'.php')) ) {
		require_once( themerex_get_file_dir('/skins/'.esc_attr($skin).'/'.esc_attr($skin).'.php') );
	}
	...
	if (($logo_icon = get_custom_option('logo_icon')) == '' && file_exists(themerex_get_file_dir('/skins/' . esc_attr($skin) . '/images/logo-icon.png')))
		$logo_icon = themerex_get_file_url('/skins/' . esc_attr($skin) . '/images/logo-icon.png');
				...
				if ($item->menu_item_parent==0) {
					if ($item->type=='taxonomy' && $item->object=='category') {
						...
						if (!empty($cur_theme) && !is_inherit_option($cur_theme))
							$items[$k]->classes[] = 'theme_'.esc_attr($cur_theme);
					}
				}
			}
		}
